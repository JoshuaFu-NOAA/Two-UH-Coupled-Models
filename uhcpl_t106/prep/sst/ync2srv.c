...
usage (void)
{
  ...
  fprintf (stderr, "usage: %s  ync-file  srv-file \n", Progname);
  fprintf (stderr,
	   "convert annual cycle monthly mean data from netCDF to SERVICE \n");
...
main (int argc, char *argv[])
{
  ...
  double *year;			/* pointer to variables read */
  int istep;
  ...
  year = read_ync (&grid, ifile, date_unit, &icode);
  ...
  if (ierr == EOF || iyear <= 0 || iyear > 9999)
    {
      fprintf (stderr, "warning: unsupported date unit: %s\n", date_unit);
      iyear = 1;
      fprintf (stderr, "         set year to: %d\n\n", iyear);
    }
  ...
  write_srv8_fblk (ostream, year, &grid, &serv);

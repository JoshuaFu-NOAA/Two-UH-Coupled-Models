!+ calculates auxiliary variables connected with the vertical finite-difference
!  scheme.
!+ $Id: auxhyb.f90,v 1.9 2002/03/14 13:38:48 m214003 Exp $
...
SUBROUTINE auxhyb(pdelp,prdelp,plnpr,palpha,ph,kdim,klen)
  ...
  USE mo_hyb,       ONLY: delpr, nlmsgl, nlmsla, nplev, nplvpa, ralpha, &
                          rdelpr, rlnpr
  ...
  !  Array arguments 
  REAL  ,INTENT(in)    :: ph(kdim,*)
  REAL  ,INTENT(inout) :: palpha(kdim,*), pdelp(kdim,*), plnpr(kdim,*), &
                          prdelp(kdim,*)

  !  Local scalars: 
  REAL    :: za, zd, zl, zr, zrd
  INTEGER :: jk, jl

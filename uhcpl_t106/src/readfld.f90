!+ read optional files
!+ $Id: readfld.f90,v 1.13 2000/03/21 12:57:12 m214003 Exp $
...
SUBROUTINE readfld
  ...
  USE mo_start_dataset, ONLY: nfl1, nfl2, nstart, nstep
  ...
  REAL, POINTER :: ftmp (:,:,:) ! global read buffer
...
!-- 1.1   Allocate memory for global fields
  ...
  IF (numfl1>0 .AND. nstep==nstart) THEN
...
CONTAINS
  ...
  SUBROUTINE readfield(nfl, numfl, ftmp)
    ...
    IF(p_pe == p_io) THEN
      ...
      IO_file_id = ini_field%nc_file_id
      ...
      ! Read field
      ALLOCATE (ftmp(nlon,numfl,ngl)); ftmp(:,:,:) = 0.

...
!+ $Id: geopot.f90,v 1.9 2002/03/14 13:38:48 m214003 Exp $
...
SUBROUTINE geopot(phi,ptv,plnpr,palpha,phis,kdim,klen)
  ...
  !  Array arguments 
  REAL ,INTENT(in)    :: palpha(kdim,*), phis(*), plnpr(kdim,*), ptv(kdim,*)
  REAL ,INTENT(inout) :: phi(kdim,*)
...
!-- 1. Integrate hydrostatic equation
  ...
  DO jk = nlevm1, 1, -1
    ...
    DO jl = 1, klen
      phi(jl,jk) = palpha(jl,jk)*ptv(jl,jk) + (plnpr(jl,ikp)-palpha(jl,ikp))* &

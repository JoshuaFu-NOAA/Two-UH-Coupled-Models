MODULE mo_machine
  ...
  ! $Id: mo_machine.f90,v 1.12 2000/08/23 09:55:51 m214003 Exp $
  ...
  INTEGER, EXTERNAL :: util_address_bit_size
...
CONTAINS
  ...
  SUBROUTINE machine_setup
    ...
    INTEGER      :: io_size, file_pointer_bit_size
    INTEGER      :: integer_byte_size, integer_io_size
    INTEGER (cp) :: intc_pointer
    ...
    intc_pointer = BIT_SIZE(intc_pointer)    
    int4_bitsize = BIT_SIZE(int4)
    int8_bitsize = BIT_SIZE(int8)

    file_pointer_bit_size = util_address_bit_size ()
    ...
    IF (intc_pointer /= file_pointer_bit_size) THEN
       IF (p_pe == p_io) THEN
          WRITE (nout, *) &
               ' There is a serious problem with the internal setup!'
          WRITE (nout, *) &
               ' The bit size of an INTEGER (CP) is ', &
               intc_pointer,                                   &
               ' and the C (FILE *) ',  file_pointer_bit_size, 'bit.'
          WRITE (nout, *) &
               ' The pbio library does not work with this setup.' 
       END IF
    END IF
    ...
    IF (mp_real .LT. 8) THEN
       IF (p_pe == p_io) THEN
          WRITE (nout, *) &
               ' There is a serious problem with the internal setup!'
          WRITE (nout, *) ' The byte size of an REAL is only ', mp_real
       END IF
